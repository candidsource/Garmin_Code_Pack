...
function db_clone_auto_cfg() {
    ...
    elif [[ "${task}" = clone_post_steps* ]] || [[ "${task}" = "final_clone_auto_config_final" ]]; then
        echo "Running post clone db auto config for task: ${task}" | tee -a ${log_file}
        script_path="${BIN_DIR}/autocfg_db_post.bin"
    else
...
function apps_clone_tasks() {
    ...
    case "${task_selector}" in
        ...
        apps_primary)
            RUN=${TGT_RUN_FS}
            PATCH=${TGT_PATCH_FS}
            script_path="${BIN_DIR}/primary_apps_fast.sh"
            ...
            ;;
        apps_ha_steps)
            script_path="${BIN_DIR}/ha_apps.sh"
            if ! get_creds; then
                log_task_status "${ENV}" "${task_type}" "${task_segment}" "${task}" failed "${log_file}"
                my_exit "${task_type}-${task_segment}_${task}" "FAILED" "${ssh_exit_code}"
            fi
        ...
        apps_secondary|apps_third)
            ...
            PATCH=${TGT_PATCH_FS}
            tier="${task#*_}"
            script_path="${BIN_DIR}/secondary_apps.sh"
    ...
    . ${BIN_DIR}/run.env"
        ...
        apps_auto_config|final_apps_auto_config_final)
            script_path="${BIN_DIR}/autocfg_apps.sh"
...
if [[ "${RESUME}" = "resume" ]]; then
    ...
    if [[ -n "${DB_CLONE_TASKS}" ]]; then
        for task in ${DB_CLONE_TASKS}; do
            task="${task/clone_/}"
            echo "FAST_CLONE: Running DB clone task: ${task}"
    ...
    if [[ -n "${APP_CLONE_TASKS}" ]]; then
        for task in ${APP_CLONE_TASKS}; do
            task="${task/apps_/}"
            echo "FAST_CLONE: Running APP clone task: ${task}"
    ...
    if [[ -n "${FINAL_TASKS}" ]]; then
        unset temp_task
        for atask in ${FINAL_TASKS}; do
            echo "FAST_CLONE: Running FINAL_TASKS task: ${atask}"
...
echo "clone_db_fast.sh completed successfully for task_type: ${TASK_TYPE}, task_segment: ${TASK_SEGMENT}, task: ${TASK}"
